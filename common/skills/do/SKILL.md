...
**Worktree base directory:** Always create worktrees under `/private/tmp/worktrees/<feature-slug>`. Never use the repo directory or any other location.

**When `base_branch` is `default`:** use `/branch` skill for branch creation; create worktrees directly with git (not the `/worktree` skill, to control the path).

| Choice | Actions (base_branch = default) |
|--------|---------|
| **Worktree + branch** | `Skill(skill="branch", args="<feature-slug>")` → `git worktree add /private/tmp/worktrees/<feature-slug> <branch-name>` → set `WORKDIR_PATH` to `/private/tmp/worktrees/<feature-slug>` |
...
|--------|---------|
| **Worktree + branch** | `git fetch origin <base_branch>` → `git worktree add --detach /private/tmp/worktrees/<feature-slug> origin/<base_branch>` → `cd /private/tmp/worktrees/<feature-slug>` → `git checkout -b <branch-name>` → set `WORKDIR_PATH` to `/private/tmp/worktrees/<feature-slug>` |
